...
function runRemote(ssh, command, { onLine, cwd } = {}) {
    ...
    const args = buildSshArgs(ssh, [sshTarget(ssh), remoteCmd]);
...
function copyToRemote(ssh, localPath, remotePath, { onLine } = {}) {
    ...
    const target = `${sshTarget(ssh)}:${remotePath}`;
...
async function probe(ssh) {
    ...
    if (result.exitCode !== 0) {
        ...
        throw new Error(`SSH connection probe failed (${sshTarget(ssh)}): ${hint}`);
    }
    return true;
}
