...
/**
 * Resolve a PathLike (string or per-OS object) to a string for the current platform.
...
function resolvePlatformPath(value, fieldName) {
    ...
    if (picked == null) {
        ...
        throw new Error(
            `${fieldName}: no path defined for this platform (${want}) ` +
            `and no "default" fallback. Defined: ${have}`
        );
    }
    return expandHome(picked);
}

module.exports = { expandHome, resolvePlatformPath, PLATFORM_ALIASES };
