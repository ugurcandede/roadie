...
const { resolvePlatformPath, pickPlatform } = require('./paths');

const VALID_TYPES = new Set(['local', 'transfer', 'remote', 'confirm']);
...
function validateSsh(s, projectName) {
    ...
    const out = { host: s.host.trim() };
    if (s.user != null) {
        if (typeof s.user !== 'string' || !s.user.trim()) {
            throw new Error(`${projectName}.ssh.user must be a non-empty string`);
        }
        out.user = s.user.trim();
    }
    if (s.port != null) {
...
function validateStep(s, projectName, j, hasSsh) {
    ...
    if (!VALID_TYPES.has(s.type)) {
        throw new Error(
            `${projectName}.steps[${j}].type is invalid: "${s.type}" (valid: local, transfer, remote, confirm)`
        );
    }
    ...
    if (s.type === 'confirm') {
        if (typeof s.prompt !== 'string' || !s.prompt.trim()) {
            throw new Error(`${projectName}.steps[${j}] (confirm): "prompt" is required`);
        }
        return { name: s.name.trim(), type: 'confirm', prompt: s.prompt };
    }
    if (s.type === 'local' || s.type === 'remote') {
        const resolvedRun = pickPlatform(s.run, `${projectName}.steps[${j}].run`);
        if (typeof resolvedRun !== 'string' || !resolvedRun.trim()) {
            throw new Error(`${projectName}.steps[${j}] (${s.type}): "run" is required`);
        }
        const out = { name: s.name.trim(), type: s.type, run: resolvedRun };
        if (s.cwd != null) {
