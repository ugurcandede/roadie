...
const DEFAULT_CONFIG = 'roadie.config.json';
const EXAMPLE_FILE = 'roadie.config.example.json';
...
${C.bold}Usage${C.reset}
  ...
  roadie --init                        Write ${EXAMPLE_FILE}
...
function writeExample(target, exampleSrc) {
    if (!fs.existsSync(exampleSrc)) {
        process.stderr.write(`${C.red}Example config not found: ${exampleSrc}${C.reset}\n`);
        process.exit(1);
    }
    ...
    fs.copyFileSync(exampleSrc, target);
...
function reportValid(config) {
    ...
    let stepCount = 0;
    const byType = { local: 0, transfer: 0, remote: 0 };
    let withSsh = 0;
    let withNotify = 0;
    ...
    process.stdout.write(`  ${C.dim}steps:${C.reset} ${byType.local} local, ${byType.transfer} transfer, ${byType.remote} remote\n`);
    process.stdout.write(`  ${C.dim}${withSsh} project(s) with ssh, ${withNotify} with notify${C.reset}\n`);
}

async function run({ exampleSrc }) {
    ...
    if (flags.has('--init')) {
        writeExample(path.resolve(process.cwd(), EXAMPLE_FILE), exampleSrc);
        return;
    }
    ...
    try {
        ...
        if (e.code === 'ENOENT') {
            ...
            process.stderr.write(`${C.dim}First: roadie --init  →  copy ${EXAMPLE_FILE} to ${DEFAULT_CONFIG} and edit.${C.reset}\n`);
        } else {
